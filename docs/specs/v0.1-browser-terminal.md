...
- Advanced remote deployments are allowed as operator-managed guidance around the app, not as built-in `0.1` hosting features.
- The docs should not assume every user runs a homelab, but they should leave room for serious operators who want to place TermiWeb behind infrastructure they already trust.
