...
Version 0.1 delivers a browser-based terminal for Windows hosts that supports:
...
- An instance's auto-generated number never changes as a side effect of another instance's lifecycle; the visible numbering may have gaps. Stable numbering and shell-driven titles are detailed in [the instance-titles spec](instance-titles.md).
...
- New instances start in the configured start directory (`TERMIWEB_START_DIRECTORY`) when it names an existing directory, otherwise in the running account's home directory, never in the repository working directory. The setting exists so an auto-started server running as `SYSTEM` can open shells in a person's home instead of the `SYSTEM` profile.
- New instances start at 80 columns.
- Terminal width is shared per instance rather than per device.
- Shared terminal rows must not be derived from live device viewport churn.
- Custom column widths should derive rows from a stable `80x30` baseline shape, so `80` columns means `30` rows and other custom widths scale from that same ratio.
- Named width presets may apply explicit stable `cols x rows` pairs when that preserves useful mobile vertical context without adding viewport-driven resizing.
...
- Nothing inside the rail widens it: rail contents stay inside the rail's padding at every width, and footer actions that do not fit side by side stack instead of pushing the rail's rows past its edge.
...
- Local viewport height may affect how much of the shared terminal is comfortably visible, but it must not redefine the shared terminal geometry or force extra zoom-out just to fit rows.
...
- Each client should keep a local follow-cursor mode for the active instance so typing or returning to live use brings the active cursor/input area back into view without mutating the shared PTY.
- That local follow-cursor mode should disable itself when the user intentionally scrolls away from the live area, and re-enable when the user resumes typing or otherwise returns to the live tail.
- If a device cannot display the full shared real-screen height at once, TermiWeb may expose a separate local viewport scroll so that device can reveal more of the current shared screen without redefining the shared terminal geometry.
...
- Wheel, touch, or drag scrolling should consume the local viewport scroll first; once that local viewport reaches its top or bottom edge, additional scrolling may then flow into xterm scrollback.
...
- After a shell has started, viewport changes on attached devices should stay local instead of continuing to rewrite the live PTY rows.
- Local viewport changes may refit font size, scroll position, and surrounding layout, but they must not redefine the shared `cols x rows` geometry for the live session.
