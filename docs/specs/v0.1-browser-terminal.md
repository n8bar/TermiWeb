...
- Shared terminal rows should be derived from the shared column count using a stable `80x30` baseline shape, so `80` columns means `30` rows and other widths scale from that same ratio.
