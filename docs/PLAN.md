# TermiWeb Plan

## Current Focus

Stabilize the first usable v1 browser terminal: multi-device instance behavior, copy/select workflow, and the mobile-first shell layout.

## Milestones

- [x] M1: Repository bootstrap and docs baseline
- [x] M2: Shared terminal backend and session model
- [x] M3: Browser UI with instances, auth, and mobile controls
- [x] M4: Local verification and launch polish

## Next Action

Run real workstation/device validation against the live app, especially the browser-chosen initial fit of the desktop-style viewport on phones, the collapsed instance rail hit targets, and the 80-column shared width before adding more features.
