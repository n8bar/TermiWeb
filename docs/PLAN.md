...
- `0.2+` feature candidate: a detached session host so live instances survive server upgrades and restarts. Each instance's ConPTY, shell process, and scrollback belong to the web server process today, so replacing the server kills every shell and whatever runs in it (the 2026-09-06 install migration did exactly that, twice). A small long-lived host process would own the PTYs and history, the web server would attach to it over a named pipe, and the installer's stop-before-upgrade step would stop only the web server. Instances still would not survive a reboot, and the host itself would need to change rarely.
