...
Continue [M10 0.1.2 Select fix and titles](worklists/m10-0.1.2-select-fix-and-titles.md): items `1`, `2`, `4`, and `5` are complete and [#2](https://github.com/n8bar/TermiWeb/issues/2), [#3](https://github.com/n8bar/TermiWeb/issues/3), [#5](https://github.com/n8bar/TermiWeb/issues/5), [#6](https://github.com/n8bar/TermiWeb/issues/6), and [#7](https://github.com/n8bar/TermiWeb/issues/7) are closed. [The Windows-installer spec](specs/windows-installer.md) is approved and installer work (item `3`) is underway: the `SYSTEM` auto-start registration passed its live test on the maintainer's workstation, so the ProgramData layout, the Inno Setup script, release assembly, and the VM verification follow, then the `0.1.2` release surface (item `6`).
