...
`M5` and `M6` are complete. `M7` is now the active milestone; the README, package-root docs, site structure, screenshots, fresh local release artifact, site implementation, and milestone-calendar generation are in place. The next step is custom-domain setup, then release metadata and final download wiring around publication time.
...
Configure the root custom domain and DNS for the GitHub Pages site in the [M7 worklist](worklists/m7-release-and-download-website.md), then prepare the GitHub release notes and publication procedure while leaving the final published asset URL wiring and the live `milestones.ics` URL for the actual release step.
