...
20. Avoid vague placeholder words such as `shape` when a more concrete term would say what is actually meant. Prefer specific terms like `layout`, `structure`, `contents`, `plan`, `scope`, `behavior`, or another word that names the real thing directly.
